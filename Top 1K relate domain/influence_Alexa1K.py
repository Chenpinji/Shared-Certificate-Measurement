...
def combined_enum(domain):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        subdlist = [crtsh_res.result(), contentdict_res.result(), dnsmx_res.result()]
    subd = []
    for res in subdlist:
        subd += res
    subd = list(set(subd))
...
file_path = 'relate_dom_Alexa1K.json'
rank = 0
with open(file_path, 'w') as file:
    for i in tqdm(cadata):
        domain = i
        rank += 1
        data = {}
        data['domain'] = domain
        ...
        if nouse != 'error':
            for k in wwwSANlist['subjectAltName']:
                if k[1][0] == '*':
                    relate_list = list(set(relate_list + combined_enum(k[1][2:])))
